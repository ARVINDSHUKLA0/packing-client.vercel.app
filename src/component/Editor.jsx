...
export default function Editor({
  ...
  const [editingText, setEditingText] = useState(null); // { panelId, elId, node }
  ...
  // panel bbox ko px space mein convert karo (Konva stage coordinates)
  ...
  const makeDragBoundFunc = useCallback(
    (panelId) => (pos) => {
      ...
      if (!box) return pos;
      // element apne panel ke rectangle ke andar hi rahe
      return {
  ...
  return (
    ...
    >
      <Stage
        ...
        style={{ background: "white", boxShadow: "0 0 8px rgba(0,0,0,0.15)" }}
      >
        {/* Dieline reference layer - sirf visual guide, editable nahi */}
          ...
          {panels.map((p) => {
            ...
            return (
              <Rect
                ...
                fill={isActive ? "rgba(37,99,235,0.03)" : "transparent"}
              />
            );
          })}
        </Layer>
        ...
        <Layer>
          {/* Panel click-to-select hit area */}
          {panels.map((p) => {
            const box = panelPx[p.id];
            return (
              <Rect
          ...
          {/* Har panel ka apna clipped group - Group ko panel position par
              translate kiya hai, isliye children ka x/y ab panel-LOCAL
              coordinates hain (0,0 = panel ka top-left) */}
          {panels.map((p) => {
            const box = panelPx[p.id];
...
// portal se document.body mein render hota hai isliye CSS-transformed
// parent ke andar wala position:fixed bug yaha nahi aayega
